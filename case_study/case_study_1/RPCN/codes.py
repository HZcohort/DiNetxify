...
import sys
import time
sys.path.append(r'/public/home/houcan/project/case_study/DiNetxify-main/src')
...
path_result = r'/public/home/houcan/project/case_study/result/ltl_rpcn'

if __name__ == "__main__":
    data = dnt.DiseaseNetworkData(study_design='cohort',phecode_level=2,
                                phecode_version='1.3a')
    time1 = time.time()
    data.phenotype_data(rf'{path_case}/LTL.csv',
    ...
    phewas_result = dnt.phewas(
        ...
        n_threshold=500,                                       # Minimum proportion of cases to include
    ...
    phewas_result.to_csv(f'{path_result}/phewas_multiple.csv',index=False)
    ...
    #data.save(rf'{path_processed}/whole_60_withtra')

    #step 3
    com_strength_result = dnt.comorbidity_strength(
        data=data,                                     # DiseaseNetworkData object
        n_threshold=50,                   # Minimum proportion for comorbidity
        n_process=n_cores,                                   # Number of parallel processes
    ...
    trajectory_result.to_csv(f'{path_result}/trajectory_RPCN_match5.csv',index=False)
